...
void ParameterGenerator::generateOldCode(juce::XmlElement* el)
{
    ...
    juce::String altCodeName = el->getStringAttribute("codeName");
    if (altCodeName.length() > 0) {
        // method name differs from what we want to use for the class name
        methodName =  altCodeName;
    }
    // no, coreName just maps the parameter name, not the method name
    //else if (coreName.length() > 0) {
