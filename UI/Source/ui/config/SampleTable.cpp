...
void SampleTable::setSamples(SampleConfig* config)
{
    ...
    while (sample != nullptr) {
        SampleFile* sf = new SampleFile(juce::String(sample->getFilename()));
        files.add(sf);
...
{
    // todo: need to find a good place
    juce::File startPath("c:/dev/jucetest/UI/Source");
    ...
    chooser->launchAsync (chooserFlags, [this] (const juce::FileChooser& fc)
    {
        ...
        // if you cancel, it apparently returns an empty File
        if (file != juce::File{})
        {
...
/**
 * We magically end up here after the FileChooser finishes, unclear
 * what thread we're on.
 * Simple enough we could have just put it in the lambda.
 */
void SampleTable::fileChooserResult(juce::File file)
{
    ...
    // would be nice to support multiple selection
