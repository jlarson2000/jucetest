/**
 * An implementation of the AudioInterface and AudioStream interfaces
 * that bridge Juce to the old world.
 *
 * There was a lot of device management in here that is now obsolete and
 * won't be called by the engine.
 *
 * The initialization process is this:
 *
 * Container builds a MobiusContext which contains an AudioInterface.
 * Mobius::start passes the AudioInterface to Recorder in the constructor
 *
 * 	Recorder(class AudioInterface* ai, class MidiInterface* mi,
 *             class AudioPool* pool);
 *
 * Recorder asks for a stream, and sets a handler
 *
 * 	mStream = mAudio->getStream();
 * 	mStream->setHandler(this);
 *
 * The stream is now expected to call the handler like this:
 *
 *  class AudioHandler {
 *    virtual void processAudioBuffers(class AudioStream* stream) = 0;
 *
 * Stream just passes itself, not the buffers, so the handler has
 * to call back to the stream to get what it wants.
 * 
 ...
 *
 * So here, as interrupts come in, we need to convert the Juce buffer structure
 * with separate channel arrays into an interleavad array and save them.
 * Then call the handler, which will immediately calls us back and ask for those
...
JuceAudioInterface::JuceAudioInterface()
{
}
...
// AudioInterface
//
// Almost entirely stubbed out except for terminate() because
// I want to see where it's called.
//
//////////////////////////////////////////////////////////////////////

/**
 * Not sure when this was called or what it was supposed to do.
 * Maybe when switching devices.
 */
void JuceAudioInterface::terminate()
{
    trace("JuceAudioInterface::terminate why so negative?\n");
}
...
// for now, going with a single port of 2 channels

int JuceAudioInterface::getInputChannels()
{
    return 2;
}

int JuceAudioInterface::getInputPorts()
{
    return 1;
}


int JuceAudioInterface::getOutputChannels()
{
    return 2;
}

int JuceAudioInterface::getOutputPorts()
{
    return 1;
}

void JuceAudioInterface::setSampleRate(int i)
{
}

int JuceAudioInterface::getSampleRate()
{
    return sampleRate;
}

/**
 * This is what Mobius calls during initialization
 * to as a callback object to receive buffers.
 */
void JuceAudioInterface::setHandler(AudioHandler* h)
{
    handler = h;
}

/**
 * Shouldn't be calling these any more, I think this
 * was just when we were in control over audio devices
 * and wanted to swap between them.
 */
bool JuceAudioInterface::open()
{
    return false;
}

void JuceAudioInterface::close()
{
}

/**
 * I Guess we could have captured a PortAudio open error
 * and displayed it.
 */
const char* JuceAudioInterface::getLastError()
{
    return nullptr;
}

/**
 * This was how you controlled buffer size in PortAudio,
 * by "suggesting" a latency.  Thankfully don't have to
 * deal with that any more, and the engine doesn't
 * get to control this anyway.
 */
void JuceAudioInterface::setSuggestedLatencyMsec(int i)
{
}

/**
 * This something might want, find it and kill it
 */
int JuceAudioInterface::getInputLatencyFrames()
{
    trace("JuceAudioInterface::getInputLatencyFrames doesn't want you to know!\n");
    return 0;
}

void JuceAudioInterface::setInputLatencyFrames(int frames)
{
}

int JuceAudioInterface::getOutputLatencyFrames()
{
    trace("JuceAudioInterface::getOutputLatencyFrames doesn't want you to know!\n");
    return 0;
}

void JuceAudioInterface::setOutputLatencyFrames(int frames)
{
}

/**
 * If we gather any interesting running stats, print them.
 * Probably called from test scripts.
 */
void JuceAudioInterface::printStatistics()
{
}

/**
 * Now we get to important stuff.
 * I don't remember exactly what this was, but it has to do
 * with synchronization so it will be important once we do sync.
 */
double JuceAudioInterface::getStreamTime()
{
    return 0.0;
}

double JuceAudioInterface::getLastInterruptStreamTime()
{
    return 0.0;
}
 ...
 */
long JuceAudioInterface::getInterruptFrames()
{
 ...
 */
void JuceAudioInterface::getInterruptBuffers(int inport, float** input, 
...
AudioTime*JuceAudioInterface:: getTime()
{
    return nullptr;
}
 ...
 */
void JuceAudioInterface::prepareToPlay (int samplesPerBlockExpected, double floatSampleRate)
{
 ...
 */
void JuceAudioInterface::releaseResources()
{
}
 ...
 */
void JuceAudioInterface::getNextAudioBlock (const juce::AudioSourceChannelInfo& bufferToFill)
{
 ...
 */
void JuceAudioInterface::test(int numSamples)
{
