...
MobiusShell::MobiusShell(MobiusContainer* cont)
{
    ...
    mPendingSamplePack = nullptr;
}

MobiusShell::~MobiusShell()
{
    delete configuration;
    delete mPendingSamplePack;
 ...
 * The SampleConfig is fully loaded and we take owership.
 * This has three phases:
 *    SamplePack construction
 *      builds a 
 *    Phasing SamplePack from the shell to the Kernel
 *    
 */
void MobiusShell::installSamples(SampleConfig* samples)
{
    // work out how this phases into the kernel
    delete samples;

    SamplePack* pack = nullptr;
    // old code would check to see if there was a difference
    // before deciding whether to build a pack, can still
    // do that but should have done it earlier before we loaded them

    // convert the SampleConfig to a SamplePack, this does NOT steal the sample
    // array from the Sample, it copies into Audio blocks
    // control flow is subtle
    // SamplePack constructor
    //   calls AudioPool::newAudio passing each Sample from within the config
    //     AudioPool calls Audio(this, Sample)
    //       Audio calls install(Sample)
    //          
    pack = new SamplePack(audioPool, samples);

    if (mPendingSamplePack != nullptr) {
        // overflow
        trace("!!! SamplePack transfer overflow\n");
        delete pack;
    }
    else {
        mPendingSamplePack = pack;
    }
...
/**
 * Called eventully by the Kernel.
 */
SamplePack* MobiusShell::consumePendingSamplePack()
{
    SamplePack* pack = mPendingSamplePack;
    mPendingSamplePack = nullptr;
    return pack;
}
 ...
 */
void MobiusShell::kernelConfigure(MobiusConfig* config)
{
...
MobiusTrackState* MobiusShell::getTargetTrack(UIAction* action)
{
    int tracknum = action->scopeTrack;
    if (tracknum == 0) {
...
void MobiusShell::doSwitch(UIAction* action, int next)
{
 ...
 */ 
void MobiusShell::doReset(UIAction* action)
{
...
void MobiusShell::reset(MobiusLoopState* loop)
{
    loop->mode = ResetMode;
    loop->init();
}
 ...
 */
void MobiusShell::doRecord(UIAction* action)
{
    MobiusTrackState* track = &(state.tracks[state.activeTrack]);
    MobiusLoopState* loop = &(track->loops[track->activeLoop]);
...
void MobiusShell::play(MobiusLoopState* loop, int bufferFrames)
{
 ...
 */
void MobiusShell::notifyBeatListeners(MobiusLoopState* loop, long bufferFrames)
{
 ...
 */
void MobiusShell::simulateEvents()
{
    MobiusTrackState* track = &(state.tracks[state.activeTrack]);
    MobiusLoopState* loop = &(track->loops[track->activeLoop]);
...
MobiusEventState* MobiusShell::simulateEvent(MobiusLoopState* loop, UIEventType* type, int q)
{
 ...
 */
Setup* MobiusShell::getActiveSetup()
{
...
SetupTrack* MobiusShell::getSetupTrack(int tracknum)
{
...
Preset* MobiusShell::getTrackPreset(SetupTrack* track)
{
...
Preset* MobiusShell::getTrackPreset(int tracknum)
{
    Preset* preset = nullptr;
 ...
 */
int MobiusShell::getParameter(Parameter* p, int tracknum)
{
    int value = 0;
